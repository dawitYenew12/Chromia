...
const WalletButton = ({
  ...
  return (
    <button
      ...
      className={`w-full flex items-center justify-between p-3 text-base font-bold text-gray-900 rounded-lg border-2 transition-colors duration-200 ease-in-out ${
        isSelected
...
export default function CustomizedModal({
  ...
  const initSession = async () => {
    if (!client || !window.ethereum) {
      console.log("No client or Ethereum provider found");
      return;
    }
    const evmKeyStore = await createWeb3ProviderEvmKeyStore(window.ethereum);
    const { getAccounts, login } = createKeyStoreInteractor(client, evmKeyStore);
  ...
  return (
    <div
      className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50 rounded-lg"
      onClick={onClose}
    >
      <div
        className="bg-white p-8 rounded-lg shadow-md max-w-sm w-full relative"
      ...
      >
        ...
        <p className="text-sm text-pink-500 text-center mb-4">
          Use{" "}
          Unlock the power of our app with <span className="text-orange-600 font-bold">MetaMask</span>. Stay tuned for support for more wallets coming soon!
        </p>
        ...
        <button
          ...
          className="w-full px-4 py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 focus:ring-2 focus:ring-blue-500"
        >
          Connect Wallet
        </button>
      </div>
